...
const DesignSubmissionForm = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!designerPhoto) {
      setError("Please upload your designer photo");
      return;
    }
    if (!formData.inspiration.trim()) {
      setError("Inspiration/Tags are required");
      return;
    }
    ...
    try {
      ...
      // Upload designer photo first
      setUploadProgress('Uploading designer photo...');
      console.log('📸 Uploading designer photo...');
      const designerPhotoData = await uploadToCloudinary(designerPhoto);
      console.log('✅ Designer photo uploaded:', designerPhotoData);

      // Upload design files
      setUploadProgress(`Uploading design files (${selectedFiles.length})...`);
      console.log('🎨 Uploading design files...');
      const uploadPromises = selectedFiles.map(file => uploadToCloudinary(file));
      const uploadedFiles = await Promise.all(uploadPromises);
      console.log('✅ Design files uploaded:', uploadedFiles);
      ...
      const tags = formData.inspiration
        .split(",")
        .map(tag => tag.trim())
        .filter(Boolean);
      ...
      setUploadProgress('Submitting design...');
      ...
      if (!response.ok) {
        ...
        if (data.error) {
          if (data.error.details && Array.isArray(data.error.details)) {
            errorMessage = data.error.details.map(d => `${d.field}: ${d.message}`).join("\n");
