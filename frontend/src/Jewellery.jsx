...
// JewelleryDesignCard
// ─────────────────────────────────────────────
function JewelleryDesignCard({ design, onUpdate }) {
  const { isAuthenticated, token, user } = useAuth();
  ...
  return (
    <div className="design-card-container">
      <div className="design-card-image">
        {primaryImage && (
          <img src={primaryImage.url} alt="Jewellery" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        )}
      </div>
      ...
      <div className="design-card-content-wrapper">
        ...
        {showComments && (
          /* keep your existing comments modal here exactly */
          <></>
        )}
  ...
  const fetchJewelleryDesigns = async () => {
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINTS.DESIGNS}?category=jewellery&limit=100&sortBy=createdAt&sortOrder=desc`
      );
      const data = await response.json();
      if (data.success) {
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              width: 36, height: 36, boxShadow: '0 2px 8px #0002',
        ...
        {loading ? (
          ...
          designs.map((design) => (
            <JewelleryDesignCard
              key={design._id}
              design={design}
              onUpdate={fetchJewelleryDesigns}
            />
          ))
        )}
      </div>
